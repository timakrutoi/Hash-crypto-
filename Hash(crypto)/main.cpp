...
void test_shamir(InfInt mes = 11, InfInt k = 3, InfInt n = 5, InfInt p = 1) {
    auto proj = get_projections(k, n, mes, p);
    ...
    for (auto i = 0; i < proj.size(); ++i) {
        cout << "\t" << get<0>(proj[i]) << " : " << get<1>(proj[i]) << endl;
    }

    proj.erase(proj.begin() + 3);
    proj.erase(proj.begin() + 2);
    // proj.erase(proj.begin() + 3);

    cout << "Projections after cut :" << endl;
    for (auto i = 0; i < proj.size(); ++i) {
        cout << "\t" << get<0>(proj[i]) << " : " << get<1>(proj[i]) << endl;
    }


    auto res = get_mes(proj, p);
